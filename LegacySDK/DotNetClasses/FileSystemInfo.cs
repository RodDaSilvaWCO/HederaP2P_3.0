...
namespace Unosys.SDK
{
	[Serializable]
#if !FEATURE_CORECLR
    //[FileIOPermissionAttribute( SecurityAction.InheritanceDemand, Unrestricted = true )] // %LEGACY Unosys SDK%
#endif
    [ComVisible( true )]
#if FEATURE_REMOTING        
	...
	public abstract class FileSystemInfo : ISerializable
	{
		...
		public DateTime CreationTimeUtc
		{
			...
			get
			{
				...
				// Important note: Directory has it's own implementation of GetCreationTimeUtc
				//if (this is DirectoryInfo)
				//	return Directory.GetCreationTimeUtc( FullPath );
				//return File.GetCreationTimeUtc( FullPath );
				// But we've got a local cache of this information, so we should use it
				return _data.CreationTime.Value.ToUniversalTime();
			}
		...
		public DateTime LastAccessTimeUtc
		{
			...
			get
			{
				...
				//return DateTime.FromFileTimeUtc( fileTime );
				// Important note: Directory has it's own implementation of GetLastAccessTimeUtc
				//if (this is DirectoryInfo)
				//	return Directory.GetLastAccessTimeUtc( FullPath );
				//return File.GetLastAccessTimeUtc( FullPath );
				// But we've got a local cache of this information, so we should use it
				return _data.LastAccessTime.Value.ToUniversalTime();
			}
		...
		public DateTime LastWriteTimeUtc
		{
			...
			get
			{
				...
				//return DateTime.FromFileTimeUtc( fileTime );
				// Important note: Directory has it's own implementation of GetLastWriteTimeUtc
				//if (this is DirectoryInfo)
				//	return Directory.GetLastWriteTimeUtc( FullPath );
				//return File.GetLastWriteTimeUtc( FullPath );
				// But we've got a local cache of this information, so we should use it
				return _data.LastWriteTime.Value.ToUniversalTime();
			}
		...
		public void Refresh()
		{
			_dataInitialised = File.FillAttributeInfo( FullPath, out _data, false, false );
		}
		...
		public FileAttributes Attributes
		{
			...
			get
			{
				...
				// This should work for both files and directories
				//return File.GetAttributes( FullPath );
				// But we've got a local cache of this information, so we should use it
				return (FileAttributes) _data.Attributes.Value;
			}
#if FEATURE_CORECLR
            [System.Security.SecurityCritical] // auto-generated
#else
