...
namespace Unosys.SDK
{
	...
	public class StreamReader : System.IO.TextReader
	{
		...
		internal StreamReader( string path, Encoding encoding, bool detectEncodingFromByteOrderMarks, int bufferSize, bool checkHost )
		{
			...
			var unosysOptions = FileOptions.Asynchronous | FileOptions.SequentialScan;
			var stream = new FileStream( path, FileMode.Open, FileAccess.Read, FileShare.Read, DefaultFileStreamBufferSize, unosysOptions, Path.GetFileName( path ), false );
		...
		public bool EndOfStream
		{
			...
			{
				if (stream == null)
					throw new InvalidOperationException( "ReaderClosed - stream is null" );
		...
		internal async Task<int> ReadAsyncInternal( char[] buffer, int index, int count )
		{
			...
			System.IO.Stream tmpStream = Stream_Prop;
		...
		private System.IO.Stream Stream_Prop
		{
			get { return stream; }
		}
		...
		private class NullStreamReader : StreamReader
		{
			...
			{
				Init( System.IO.Stream.Null );
			}

			public override System.IO.Stream BaseStream
			{
				get { return System.IO.Stream.Null; }
			}
