	...
	public static class File
	{
		...
		public static void Copy( string sourceFileName, string destFileName )
		{
			if (sourceFileName == null)
				throw new ArgumentNullException( "sourceFileName", "ArgumentNull_FileName" );
			if (destFileName == null)
				throw new ArgumentNullException( "destFileName", "ArgumentNull_FileName" );
			if (sourceFileName.Length == 0)
				throw new ArgumentException( "Argument_EmptyFileName", "sourceFileName" );
		...
		internal static string InternalCopy( string sourceFileName, string destFileName, bool overwrite, bool checkHost )
		{
			...
			var destMode = overwrite ? FileMode.OpenOrCreate : FileMode.Create;
			var destShare = overwrite ? FileShare.None : FileShare.Write;
			using (var sourceFile = new FileStream( fullSourceFileName, FileMode.Open, FileAccess.Read, FileShare.Read ))
			using (var destFile = new FileStream( fullDestFileName, destMode, FileAccess.Write, destShare ))
			{
		...
		// directory specified doesn't exist.
		//
		// Your application must have Create, Read, and Write permissions to the file.
		[ResourceExposure( ResourceScope.Machine )]
		[ResourceConsumption( ResourceScope.Machine )]
		...
		internal static void InternalDelete( string path, bool checkHost )
		{
			...
			var handle = API.OpenSetAsync( fullPath, (uint) FileMode.Open, (uint) FileAccess.ReadWrite, (uint) FileOptions.Asynchronous, (uint) FileShare.Delete ).Result;
			var r = API.DeleteSetAsync( handle ).Result;
			/*if (!r)
			{
			...
			if (!r )
				throw new IOException( "Failure in Delete" );
		}
		...
		internal static bool InternalExists( string path )
		{
			...
			try
			{
				// These will only succeed if path represents a valid set (that we can open)
				// TODO: Check for file existence without the requirement of being able to open the file
				var handle = API.OpenSetAsync( path, (uint) FileMode.Open, (uint) FileAccess.Read, (uint) FileOptions.Asynchronous, (uint) FileShare.Read ).Result;
				var attribs = API.GetSetAttributesAsync( handle ).Result;

				// If it's a valid set and it's not a Directory, then it must be a File!
				result = (attribs & (uint) FileAttributes.Directory) == 0;
			}
			catch
			{
				// Exception thrown due to file not existing, or not being openable, either way we'll call it a "file doesn't exist" problem
				// Note that these are actually entirely seperate things though, and we merely lack the capability to distinguish
				// For this reason, we eat all errors here and assume "false" is the correct return value
				System.Diagnostics.Debug.WriteLine( "File.InternalExists: Exception thrown trying to open {0}, we'll assume that's because it doesn't exist", path );
			}
		...
		public unsafe static void SetCreationTimeUtc( string path, DateTime creationTimeUtc )
		{
			...
			using (OpenFile( path, FileAccess.Write, out handle ))
			{
				...
				var r = API.SetSetCreationTimeAsync( handle, creationTimeUtc.ToUniversalTime() ).Result;
				/*if (!r)
				{
					int errorCode = Marshal.GetLastWin32Error();
					__Error.WinIOError( errorCode, path );
				}*/
				if (r != APIOperationResponseCode.OK)
					throw new IOException( "Failed to set CreationTime" );
			}
		}
		...
		private static DateTime InternalGetCreationTimeUtc( string path, bool checkHost )
		{
			...
			var fileHandle = API.OpenSetAsync( path, (uint) FileMode.Open, (uint) FileAccess.Read, (uint) FileOptions.Asynchronous, (uint) FileShare.Read ).Result;
			var result = API.GetSetCreationTimeAsync( fileHandle ).Result;
			var closeResult = API.CloseSetAsync( fileHandle ).Result;

			//if (dataInitialised != 0)
			//	__Error.WinIOError( closeResult, fullPath );
			if (!closeResult )
				throw new IOException( "Failure to close?" );

			//long dt = ((long) (data.ftCreationTimeHigh) << 32) | ((long) data.ftCreationTimeLow);
			//return DateTime.FromFileTimeUtc( dt );
			// Should already be UTC, but let's make certain
		...
		public unsafe static void SetLastAccessTimeUtc( string path, DateTime lastAccessTimeUtc )
		{
			...
			using (OpenFile( path, FileAccess.Write, out handle ))
			{
				...
				var r = API.SetSetLastAccessTimeAsync( handle, lastAccessTimeUtc.ToUniversalTime() ).Result;
				/*if (!r)
				{
					int errorCode = Marshal.GetLastWin32Error();
					__Error.WinIOError( errorCode, path );
				...
				if (r != APIOperationResponseCode.OK)
					throw new IOException( "Failure setting LastAccessTime" );
			}
		}
		...
		private static DateTime InternalGetLastAccessTimeUtc( string path, bool checkHost )
		{
			...
			//int dataInitialised = FillAttributeInfo( fullPath, ref data, false, false );
			var fileHandle = API.OpenSetAsync( path, (uint) FileMode.Open, (uint) FileAccess.Read, (uint) FileOptions.Asynchronous, (uint) FileShare.Read ).Result;
			var result = API.GetSetLastAccessTimeAsync( fileHandle ).Result;
			var closeResult = API.CloseSetAsync( fileHandle ).Result;
		...
		public unsafe static void SetLastWriteTimeUtc( string path, DateTime lastWriteTimeUtc )
		{
			...
			using (OpenFile( path, FileAccess.Write, out handle ))
			{
				...
				var r = API.SetSetLastWriteTimeAsync( handle, lastWriteTimeUtc ).Result;
				/*if (!r)
				{
					int errorCode = Marshal.GetLastWin32Error();
					__Error.WinIOError( errorCode, path );
				...
				if (r != APIOperationResponseCode.OK)
					throw new IOException( "Failed setting LastWriteTime" );
			}
		}
		...
		private static DateTime InternalGetLastWriteTimeUtc( string path, bool checkHost )
		{
			...
			//int dataInitialised = FillAttributeInfo( fullPath, ref data, false, false );
			var fileHandle = API.OpenSetAsync( path, (uint) FileMode.Open, (uint) FileAccess.Read, (uint) FileOptions.Asynchronous, (uint) FileShare.Read ).Result;
			var result = API.GetSetLastWriteTimeAsync( fileHandle ).Result;
			var closeResult = API.CloseSetAsync( fileHandle ).Result;
...
*/
			...
			var fileHandle = API.OpenSetAsync( path, (uint) FileMode.Open, (uint) FileAccess.Read, (uint) FileOptions.Asynchronous, (uint) FileShare.Read ).Result;
			var result = API.GetSetAttributesAsync( fileHandle ).Result;
			var closeResult = API.CloseSetAsync( fileHandle ).Result;
			//if (dataInitialised != 0)
			//	__Error.WinIOError( dataInitialised, fullPath );
			if (!closeResult )
		...
		public static void SetAttributes( string path, FileAttributes fileAttributes )
		{
			...
			var handle = API.OpenSetAsync( fullPath, (uint) FileMode.Open, (uint) FileAccess.Read, (uint) FileOptions.Asynchronous, (uint) FileShare.Read ).Result;
			var r = API.SetSetAttributesAsync( handle, (uint) fileAttributes ).Result;
			var closeResult = API.CloseSetAsync( handle ).Result;
			/*if (!r)
			{
				int hr = Marshal.GetLastWin32Error();
			...
			if (r != APIOperationResponseCode.OK || !closeResult )
				throw new IOException( "Failed setting Attributes" );
		}
		...
		private static byte[] InternalReadAllBytes( string path, bool checkHost )
		{
			...
			using (FileStream fs = new FileStream( path, FileMode.Open, FileAccess.Read, FileShare.Read,
				FileStream.DefaultBufferSize, FileOptions.Asynchronous, Path.GetFileName( path ), false ))//, false, checkHost ))
			{
				// Do a blocking read
				int index = 0;
		...
		[ResourceExposure( ResourceScope.Machine )]
		[ResourceConsumption( ResourceScope.Machine )]
		//internal static int FillAttributeInfo( string path, ref FileAttributes data, bool tryagain, bool returnErrorOnNotFound )
		internal static int FillAttributeInfo( string path, out SetPropertyDTO data, bool tryagain, bool returnErrorOnNotFound )
		{
			...
			if (tryagain) // someone has a handle to the file open, or other error
			{
				...
				//{
				//bool error = false;
				throw new NotImplementedException( "FillAttributeInfo - TryAgain capability not yet implemented" );
				/*SafeFindHandle handle = Win32Native.FindFirstFile( tempPath, findData );
				try
				{ // Don't need the try if we haven't changed the error mode
					if (handle.IsInvalid)
					{
						error = true;
				...
				{ // Don't need the finally when you don't need the try
			...
			else
			{
				...
				try
				{
					...
					var fileHandle = API.OpenSetAsync( path, (uint) FileMode.Open, (uint) FileAccess.Read, (uint) FileOptions.Asynchronous, (uint) FileShare.Read ).Result;
					data = API.GetSetPropertiesAsync( fileHandle ).Result;
					var closeResult = API.CloseSetAsync( fileHandle ).Result;
					if (closeResult )
						success = true;
				}
				...
				{
					System.Diagnostics.Debug.Print( "File.GetAttributeInfo - Exception thrown fetching properties for set: {0}", e );
					throw;
				}
				/*
				finally
				...
				if (!success)
				{
						...
						dataInitialised != Win32Native.ERROR_NOT_READY)  // floppy device not ready
					{
						...
						return FillAttributeInfo( path, out data, true, returnErrorOnNotFound );
					}
					else
					{
						if (!returnErrorOnNotFound)
						{
							...
							dataInitialised = 0;
							//data.fileAttributes = -1;
							//data.Attributes = (uint) FileAttributes.ErrorNotFound;
		...
		/// <summary>
		/// Methods of File that were not given a working concrete implementation for Unosys
		/// </summary>
...
#else
			throw new NotImplementedException( "Unosys does not yet support File.Decrypt" );
			/*string fullPath = Path.GetFullPathInternal( path );
		...
		public static void Encrypt( string path )
		{
            ...
            throw new PlatformNotSupportedException(Environment.GetResourcestring("PlatformNotSupported_RequiresNT"));
#else
			throw new NotImplementedException( "Unosys does not yet support File.Encrypt" );
			/*string fullPath = Path.GetFullPathInternal( path );
			FileIOPermission.QuickDemand( FileIOPermissionAccess.Read | FileIOPermissionAccess.Write, fullPath, false, false );
